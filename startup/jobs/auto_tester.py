"""
Headless MT5 Strategy Tester Automation and Feedback Parsing.
"""
import os
import shutil
import subprocess
...
MT5_TERMINAL_PATH = os.environ.get(
    "MT5_PATH", 
    'env WINEPREFIX="/home/infoscience/.wine" wine-stable "C:\\users\\Public\\Desktop\\FBS MetaTrader 5.lnk"'
)
...
async def run_headless_optimization(asset: str) -> str:
    """Generates an INI file and runs MT5 headlessly via Wine using a non-blocking async process."""
...
"""
    ...
    # Non-blocking async process creation
    proc = await asyncio.create_subprocess_shell(
        f"{MT5_TERMINAL_PATH} /config:{ini_path}",
    ...
    if not os.path.exists(report_path):
        report_path = f"/tmp/{asset}_report.xml"
    return report_path
